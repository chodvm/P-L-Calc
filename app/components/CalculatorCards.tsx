...
export default function CalculatorCards() {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      // Load active staff
      const staffRes = await s
        .from('staff')
        .select('id, full_name, role, base_hourly_rate, default_daily_hours')
        .eq('active', true)
        .order('role')
        .order('full_name')
      setStaff(staffRes.data ?? [])

      // Org average reimbursement
      ...
      // Daily fixed cost (sum of v_fixed_costs_daily)
      const fixed = await s.from('v_fixed_costs_daily').select('daily_amount')
      const totalFixed = (fixed.data ?? []).reduce((acc: number, r: any) => acc + Number(r.daily_amount || 0), 0)
  ...
  const selectedStaff = useMemo(() => {
    ...
    return [...selectedPhysicians, ...selectedOthers]
      .map(id => map.get(id)!)
      .filter(Boolean)
  ...
  const laborCost = useMemo(() => (
    selectedStaff.reduce((sum, s) => (
      sum + (Number(s.base_hourly_rate) * Number(s.default_daily_hours ?? 8))
    ), 0)
  ), [selectedStaff])
  ...
  const saveDay = async () => {
    ...
    try {
      ...
      const { error } = await s.rpc('save_daily_log_and_assignments', {
        p_org: orgId,
        p_work_date: workDate,          // <-- chosen date
    ...
    <div className="grid gap-6 md:grid-cols-2">
      ...
      <div className="space-y-4">
        <div className="rounded-2xl border p-4">
          ...
          <div className="text-xs text-slate-500 mb-2">Select one or more</div>
          <select
            multiple
            className="w-full border rounded-xl p-2 h-40"
            value={selectedPhysicians}
            onChange={e => setSelectedPhysicians(Array.from(e.target.selectedOptions).map(o => o.value))}
          >
            {physicians.map(p => (
              <option key={p.id} value={p.id}>
                {p.full_name} — ${p.base_hourly_rate}/hr × {p.default_daily_hours ?? 8}h
              </option>
            ))}
          </select>
        </div>
        ...
        <div className="rounded-2xl border p-4">
          ...
          <div className="text-xs text-slate-500 mb-2">Select as needed</div>
          <select
            multiple
            className="w-full border rounded-xl p-2 h-56"
            value={selectedOthers}
            onChange={e => setSelectedOthers(Array.from(e.target.selectedOptions).map(o => o.value))}
          >
            {others.map(p => (
              <option key={p.id} value={p.id}>
                {p.full_name} ({p.role}) — ${p.base_hourly_rate}/hr × {p.default_daily_hours ?? 8}h
              </option>
            ))}
          </select>
        </div>
      ...
      <div className="space-y-4">
        <div className="rounded-2xl border p-4">
          ...
          <div className="grid gap-3">
            ...
            <div className="text-sm text-slate-600">
              Org avg reimbursement (auto): <span className="font-semibold">${(avgReimb || 0).toFixed(2)}</span>
            </div>
          </div>
        </div>
